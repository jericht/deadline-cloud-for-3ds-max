...
class DefaultMaxHandler:
    ...
    def start_render(self, data: dict) -> None:
        ...
        try:
            ...
            # Set rendOutputFilename and rendSaveFile instead of passing outputFile
            # to rt.render, to prevent 3ds Max from adding its own frame padding.
            if renderer_handles_output:
                rt.render(camera=self.camera_node, quiet=True)
            else:
                # rt.rendOutputFilename = output_path
                # rt.rendSaveFile = True
                # self.log_to_console(f"Set 3dsMax rendOutputFilename to {output_path}")
